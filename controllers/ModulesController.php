...
use nhockizi\cms\models\CopyModuleForm;
...
class ModulesController extends \nhockizi\cms\components\Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('nhockizi_cms', 'Module created'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('nhockizi_cms', 'Module updated'));
                }
                else{
    ...
    public function actionSettings($id)
    {
        ...
        if (Yii::$app->request->post('Settings')) {
            ...
            if($model->save()){
                $this->flash('success', Yii::t('nhockizi_cms', 'Module settings updated'));
            }
            else{
                $this->flash('error', Yii::t('nhockizi_cms', Yii::t('nhockizi_cms', 'Update error. {0}', $model->formatErrors())));
            }
            return $this->refresh();
        }
        else {
    ...
    public function actionRestoreSettings($id)
    {
        if(($model = Module::findOne($id))){
            $model->settings = '';
            $model->save();
            $this->flash('success', Yii::t('nhockizi_cms', 'Module default settings was restored'));
        } else {
            $this->flash('error', Yii::t('nhockizi_cms', 'Not found'));
        }
        return $this->back();
            ...
            if(Yii::$app->request->isAjax){
                ...
                foreach(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($newModuleFolder)) as $file => $object){
                    if(!$object->isDir()){
                        ...
                        $fileContent = str_replace("Yii::t('nhockizi_cms/".$module->name, "Yii::t('nhockizi_cms/".$formModel->name, $fileContent);
                ...
                foreach(glob($newModuleFolder . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . '*.php') as $modelFile){
                    ...
                    $newTableName = str_replace('nhockizi_cms', 'app', $newTableName);
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('nhockizi_cms', 'Module deleted'));
    }
