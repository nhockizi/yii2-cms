...
<?php if(!empty($parent)) : ?>
    <div class="form-group field-category-title required">
        <label for="category-parent" class="control-label"><?= Yii::t('nhockizi_cms', 'Parent category') ?></label>
        <select class="form-control" id="category-parent" name="parent">
            <option value="" class="smooth"><?= Yii::t('nhockizi_cms', 'No') ?></option>
...
<?php if($settings['categoryThumb']) : ?>
    <?php if($model->image) : ?>
        ...
        <a href="<?= Url::to(['/admin/'.$this->context->moduleName.'/a/clear-image', 'id' => $model->primaryKey]) ?>" class="text-danger confirm-delete" title="<?= Yii::t('nhockizi_cms', 'Clear image')?>"><?= Yii::t('nhockizi_cms', 'Clear image')?></a>
    <?php endif; ?>
