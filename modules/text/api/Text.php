<?php
namespace nhockizi\cms\modules\text\api;

use Yii;
use nhockizi\cms\components\API;
use nhockizi\cms\helpers\Data;
use yii\helpers\Url;
use nhockizi\cms\modules\text\models\Text as TextModel;
use yii\helpers\Html;

/**
 * Text module API
 * @package nhockizi\cms\modules\text\api
 *
...
class Text extends API
{
    ...
    private function notFound($id_slug)
    {
        ...
        if(!Yii::$app->user->isGuest && preg_match(TextModel::$SLUG_PATTERN, $id_slug)){
            $text = Html::a(Yii::t('nhockizi_cms/text/api', 'Create text'), ['/admin/text/a/create', 'slug' => $id_slug], ['target' => '_blank']);
        }
