<?php
namespace nhockizi\cms\modules\feedback\controllers;
...
class AController extends Controller
{
        ...
        if($postData) {
            if(filter_var(Setting::get('admin_email'), FILTER_VALIDATE_EMAIL))
            {
                ...
                if($model->sendAnswer()){
                    ...
                    $model->save();
                    $this->flash('success', Yii::t('nhockizi_cms/feedback', 'Answer successfully sent'));
                }
                else{
                    $this->flash('error', Yii::t('nhockizi_cms/feedback', 'An error has occurred while sending mail'));
                }
            }
            else {
                $this->flash('error', Yii::t('nhockizi_cms/feedback', 'Please fill correct `Admin E-mail` in Settings'));
            }
            ...
            if(!$model->answer_text) {
                $model->answer_subject = Yii::t('nhockizi_cms/feedback', $this->module->settings['answerSubject']);
                if ($this->module->settings['answerHeader']) $model->answer_text = Yii::t('nhockizi_cms/feedback', $this->module->settings['answerHeader']) . " " . $model->name . ".\n";
                if ($this->module->settings['answerFooter']) $model->answer_text .= "\n\n" . Yii::t('nhockizi_cms/feedback', $this->module->settings['answerFooter']);
            }
    ...
    public function actionSetAnswer($id)
    {
        ...
        if($model === null){
            ...
            $model->status = Feedback::STATUS_ANSWERED;
            if($model->update()) {
                $this->flash('success', Yii::t('nhockizi_cms/feedback', 'Feedback updated'));
            }
            else{
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('nhockizi_cms/feedback', 'Feedback deleted'));
    }
}